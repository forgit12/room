...
<script>
  ...
  export default {
    ...
    watch: {
      historyEntries(newVal) {
        this.blueBarChart.chartData.labels = newVal.map((item) => item.CallId)
        this.blueBarChart.chartData.datasets[0].data = newVal.map((item) => item.Duration)

        this.$refs.barChart.renderChart(this.blueBarChart.chartData, this.blueBarChart.extraOptions);
      }
    },
    ...
    methods: {
      ...
      dial() {
        if (this.wsconnection) {
          this.wsconnection.command('Dial', {Number: 'balexey@cisco.com'})
            .then((call) => {
              ...
              setTimeout(() => {
                ...
                this.wsconnection.command('Call Disconnect', {CallId: call.CallId})
              }, delay * 2000);
            })
        }
      },
      ...
      initConnection() {
        ...
        this.wsconnection.event.on('CallDisconnect', (event) => {
          ...
          this.historyEntries.unshift(event);
        });
